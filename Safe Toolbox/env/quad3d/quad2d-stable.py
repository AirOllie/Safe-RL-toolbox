...
class Quad2D(gym.Env):
    ...
    def __init__(
        ...
        self._init_state = {
            "low": [-0.75, 0, -np.pi/4, -1, -1, -1],
            "high": [0.75, 1, np.pi/4, 1, 1, 1]
        }
    ...
    def unsafe(self):
        ...
        #     done = True
            # print("b")
        if np.sqrt(self.x**2+self.z**2+self.theta**2+self.vx**2+self.vz**2+self.dtheta**2)>= 7:
            done = True
            # print("c")
        return done
    ...
    def success(self):
        ...
        if np.abs(self.theta)<1 and np.sqrt((self.x-self.xt)**2 + (self.z-self.zt)**2)< 0.3:
            done = True
        return done
    ...
    def step(self, action):
        ...
        self.x = self.x + self.dt * self.vx + 0.5 * np.sin(self.theta)*(u[0]+u[1])/self.m * (self.dt**2)
        self.z = self.z + self.dt * self.vz + 0.5 * (-self.g + np.cos(self.theta)*(u[0]+u[1])/self.m) * (self.dt**2)
        self.theta = self.theta + self.dt * self.dtheta + 0.5 * self.rr * (u[0]-u[1]) / self.I * (self.dt**2)
        self.vx = self.vx + self.dt * np.sin(self.theta)*(u[0]+u[1])/self.m
        self.vz = self.vz + self.dt * (-self.g + np.cos(self.theta)*(u[0]+u[1])/self.m)
        self.dtheta = self.dtheta + self.dt * self.rr * (u[0]-u[1]) / self.I
        self.state = np.array([self.x, self.z, self.theta, self.vx, self.vz, self.dtheta])

        self.r = 5 * (self.x-self.xt)**2 + (self.z-self.zt)**2 + 5 * self.theta**2
        self.r = -self.r
        self.out = 0
        if self.unsafe():
            self.done = True
            self.r = -400
        elif self.step_num == 3000:
            self.done = True
            self.r = -400
        elif self.success():
            self.done = True

...
class Crazyflie(gym.Env):
    def __init__(self):
        ...
        START = time.time()
        # high_s = np.array([4, 4, 4, 2, 2, 2, np.pi, np.pi, np.pi,])
        high_s = np.array([4, 4, 4, 2, 2, 2,])
        low_a = np.array([0, 0, 0, 0, 0, 0])
        high_a = np.array([60, 60, 60, 10, 10, 10])
        self.action_space = spaces.Box(low=low_a, high=high_a, dtype=np.float32)
        high_v = np.array([8, 8, 8])
    ...
    def unsafe(self):
        ...
        if np.sqrt(self.x ** 2 + self.y ** 2 + (self.z - 0.5) ** 2) >= 3.5:
            done = True
        if np.sqrt(self.roll ** 2 + self.pitch ** 2) >= np.pi / 3:
            done = True
        return done
    ...
    def step(self, action):
        ## 控制量坐标系转换
        action[0] = 5 + 10 * action[0]
        action[1] = 5 + 10 * action[1]
        action[2] = 5 + 10 * action[2]
        action[3] = 5 * action[3]
        action[4] = 5 * action[4]
        action[5] = 5 * action[5]
        action = np.clip(action, self.action_space.low, self.action_space.high)
        v_d = np.zeros(3)
        v_d[0] = -action[0] * self.state[0] - action[3] * self.state[3]
        v_d[1] = -action[1] * self.state[1] - action[4] * self.state[4]
        v_d[2] = -action[2] * self.state[2] - action[5] * self.state[5]
        v_d = np.clip(v_d, self.v_space.low, self.v_space.high)
        u = {
            str(0): np.array(
                [
                    v_d[0],
                    v_d[1],
                    v_d[2],
                    np.sqrt(v_d[0] ** 2 + v_d[1] ** 2 + v_d[2] ** 2),
                ]
            )
        }
        for i in range(5):
        ...
        self.r = np.sqrt(self.x ** 2 + self.y ** 2 + (self.z - 0.5) ** 2)
        self.r = -self.r
        self.out = 0
        # if (self.z - 0.5) < 0:
        #     print(s)
...
if __name__ == "__main__":
    ...
    for ii in range(3):
        ...
        for i in range(6000):
            time.sleep(0.01)
            action = np.array([10, 10, 30, 1, 1, 0.5])

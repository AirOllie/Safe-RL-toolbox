...
class Crazyflie(gym.Env):
    ...
    def reset(self):
        ...
        while self.flat_start:
            ...
            self.x = self.INIT_XYZS[0]
            self.z = self.INIT_XYZS[2]
            self.vx = 0
            self.vz = 0
        ...
        self.state = np.array([self.x, (self.z - 0.5), self.vx, self.vz,])
        return self.state
    ...
    def unsafe(self):
        ...
        if self.z < 0.2:
            ...
            # print("a")
        if self.x < -0.5 and self.x > -1.0 and self.z < 1 and self.z > 0.1:
            done = True
            # print("b")
        if self.x < 1 and self.x > 0 and self.z < 1.8 and self.z > 1.3:
            done = True
            # print("b")
        if (
            np.sqrt(self.x ** 2 + (self.z - 0.5) ** 2 + self.vx ** 2 + self.vz ** 2)
            >= 5
        ):
            done = True
            # print("c")
    ...
    def success(self):
        ...
        if np.sqrt((self.x) ** 2 + (self.z - 0.5) ** 2) < 0.3:
            done = True
        return done
    ...
    def step(self, action):
        ...
        u = {
            str(0): np.array(
                [action[0], 0, action[1], np.sqrt(action[0] ** 2 + action[1] ** 2),]
            )
        }
        for i in range(5):
            obs, reward, done, info = self.env.step(u)

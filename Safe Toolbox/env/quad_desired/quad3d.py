...
class QuadrotorEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.max_angle = (
            40 * math.pi / 180
        )  # you can specify the maximum commanded angle here
        ...
        high_a = np.array([0.05, 0.05, 0.02, 0.1])
    ...
    def quadEOM(self, t, s, F, M):
        ...
        prop_thrusts_clamped = np.maximum(
            np.minimum(prop_thrusts, self.max_F / 4), self.min_F / 4
        )
        B = [
            [1, 1, 1, 1],
        ...
        M = np.reshape(
            [
                np.dot(B[1:3], prop_thrusts_clamped)[0],
                np.dot(B[1:3], prop_thrusts_clamped)[1],
                M[2],
            ],
            [3],
        )
        ...
        # Acceleration
        accel = (
            1 / self.m * (np.dot(wRb, [[0], [0], [F]]) - [[0], [0], [self.m * self.g]])
        )
        # Angular velocity
        ...
        qdot = np.dot(
            ...
            quat,
        ) + np.multiply(K_quat * quaterror, quat)
    ...
    def controller(self, desired_state, x):
        ...
        acc_des = (
            acc_des
            + Kd * (np.subtract(vel_des, vel))
            + Kp * (np.subtract(pos_des, pos))
        )
        #
        #  Desired roll, pitch and yaw
        phi_des = (
            1 / self.g * (acc_des[0] * np.sin(yaw_des) - acc_des[1] * np.cos(yaw_des))
        )
        theta_des = (
            1 / self.g * (acc_des[0] * np.cos(yaw_des) + acc_des[1] * np.sin(yaw_des))
        )
        psi_des = yaw_des
        #
        ...
        M_ = np.multiply(KdM, (np.subtract(pqr_des, omega))) + np.multiply(
            KpM, (np.subtract(euler_des, euler))
        )
        M = np.dot(self.I, M_)
        return F, M
    ...
    def step(self, a):
        ...
        desired_state = [
            [a[0] + self.state[0], a[1] + self.state[1], a[2] + self.state[2]],
            [a[0], a[1], a[2]],
            [0, 0, 0],
            0,
            0,
        ...
        s_ = self.state[0:13] + self.dt * s_dot

        # s_dot= xyzdot,acc,qdot4,pqrdot3= 13维
        # 判断是否偏离太多
        target_ = trajectory("hover", self.t)
        ...
        if s_[2] < -0.3:
            ...
            r = -2000
        if np.abs(s_[0]) > 3.5 or np.abs(s_[1]) > 3.5 or np.abs(s_[2]) > 3.5:
            done = True
            r = -2000
        if self.step_num == 1000:
            done = True
        return self.state, r, done, out
    ...
    def reset(self):
        ...
        self.state[0] = -3 + 6 * np.random.rand()
        self.state[1] = -3 + 6 * np.random.rand()
        self.state[2] = 0.5 + 2.5 * np.random.rand()
        self.t = 0
        return self.state
...
def RPYtoRot_ZXY(phi, theta, psi):
    R = [
        [
            ...
            - math.sin(phi) * math.sin(psi) * math.sin(theta),
            math.cos(theta) * math.sin(psi)
            + math.cos(psi) * math.sin(phi) * math.sin(theta),
            -math.cos(phi) * math.sin(theta),
        ],
        [-math.cos(phi) * math.sin(psi), math.cos(phi) * math.cos(psi), math.sin(phi)],
        [
            math.cos(psi) * math.sin(theta)
            + math.cos(theta) * math.sin(phi) * math.sin(psi),
            math.sin(psi) * math.sin(theta)
            - math.cos(psi) * math.cos(theta) * math.sin(phi),
...
def trajectory(name, t):
    if name == "circle":
        ...
        def pos_from_angle(a):
            ...
            pos = [
                np.multiply(radius, np.cos(a)),
                np.multiply(radius, np.sin(a)),
...
def Hover(state):
    ...
    r = np.sign(stability) * ((stability) ** 2) + 0.5 * np.sign(distance) * (
        (distance) ** 2
    )

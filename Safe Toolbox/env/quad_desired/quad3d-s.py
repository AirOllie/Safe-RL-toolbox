...
class Quad3D(gym.Env):
    def __init__(
        ...
        self._init_state = {
            "low": [-1, -1, -1, -2, -2, -2, -0.2 * np.pi, -0.2 * np.pi, -0.2 * np.pi],
            "high": [1, 1, 1, 2, 2, 2, 0.2 * np.pi, 0.2 * np.pi, 0.2 * np.pi],
        }
        self._state_low = np.array(
            [-1, -1, -1, -2, -2, -2, -0.2 * np.pi, -0.2 * np.pi, -0.2 * np.pi]
        )

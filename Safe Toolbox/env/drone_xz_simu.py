...
class DroneXZ(Env, utils.EzPickle):
    def __init__(self):
        ...
        self.area_left_bottom = {"low": np.array([-2.5, 0, 0.2]), "high": np.array([-1, 0, 1])}
        self.area_middle_top = {"low": np.array([-2.5, 0, 1]), "high": np.array([0, 0, 1.8])}
    ...
    def reset(self):
        ...
        if self.i_episode < 1000:
            if ran_int[0] in [1, 2]:
                init_state = self.area_right_bottom
            elif ran_int[0] in [3, 4, 5]:
                init_state = self.area_left_bottom
            else:
                init_state = self.area_middle_top
        else:
            if ran_int[0] in [1, 2, 3, 4]:
                init_state = self.area_middle_top
            else:
                init_state = self.area_left_bottom
        # init_state = {"low": np.array([-1.5, 0, 0.75]), "high": np.array([-1, 0, 1])}
    ...
    def unsafe(self):
        x = self.obs_drone_state[0]
        z = self.obs_drone_state[1]
        if z <= 0.2 or z >= 1.8 or x>1 or x<-2.5:
            return True
        if x <= -0.5 and x >= -1.0 and z <= 1 and z >= 0.1:
            return True
        if x <= 1 and x >= 0 and z <= 1.8 and z >= 1.3:
            return True
        # TODO add vel constr.
    ...
    def get_observation_space(self):        
        obs_lower_bound = np.array([-2, -2, -1, -1])
        obs_upper_bound = np.array([2, 2, 1, 1])

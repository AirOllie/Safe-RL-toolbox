        ...
        if not self.images:
            ...
            if self.encoding:
                ...
                self.safety_critic_target = QNetworkConstraintCNN(
                    obs_space, ac_space.shape[0], hidden_size,
                    args.env_name).to(self.device)
        if args.Q_risk_ablation:
            print('Using Q_risk_ablation')
            if args.method_name == 'RSPO':
                # self.safety_critic = torch.load("saved_model/safety_critic_kine_car_RSPO_ablation_epi500_seed142.pkl")
                self.safety_critic = torch.load("saved_model/safety_critic_drone_xz_RSPO_epi1500_seed43.pkl")
            elif args.method_name == 'SQRL':
                # self.safety_critic = torch.load("saved_model/safety_critic_kine_car_SQRL_ablation_epi500_seed142.pkl")
                self.safety_critic = torch.load("saved_model/safety_critic_drone_xz_SQRLv1_epi1500_seed43.pkl")

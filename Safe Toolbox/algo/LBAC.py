...
class LBAC(Base):
    def __init__(self, env, env_name='drone_xz', cnn=False, lr=3e-4,
                 ...
                 nu_start=1e3, nu_end=0, use_LBAC=True, lambda_LBAC=0.01, use_RCPO=False, lambda_RCPO=0.01):

        super(LBAC, self).__init__(env, env_name, cnn, lr,
    ...
    def _agent_setup(self, env):
        agent = SAC(env.observation_space,
                    ...
                    self.args,
                    self.logdir,
                    tmp_env=env if self.eval else make_env(self.env_name))
        return agent

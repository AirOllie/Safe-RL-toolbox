...
class SQRL:
    def __init__(self, env, env_name='drone_xz', method_name='SQRLv1', logdir='drone_xz', logdir_suffix='SQRL', cuda=False, cnn=False, lr=3e-4,
                 ...
                 LBAC=False, lambda_LBAC=0.01, Q_risk_ablation=False, constraint_reward_penalty=0,
                 DGD_constraints=False, use_constraint_sampling=False, nu=0.01, update_nu=True, nu_schedule=False,
                 nu_start=1e3, nu_end=0, RCPO=False, lambda_RCPO=0.01):

        class arguments:
            def __init__(self, env_name='drone_xz', method_name='SQRLv1', logdir='drone_xz', logdir_suffix='SQRL', cuda=False, cnn=False, lr=3e-4,
        ...
        # args = {'env_name': env_name, 'method_name':method_name, 'logdir':logdir, 'logdir_suffix':logdir_suffix, 'cuda':cuda, 'cnn':cnn, 'lr':lr,
        #        'updates_per_step':updates_per_step, 'start_steps':start_steps, 'target_update_interval':target_update_interval, 'policy':policy, 'eval':eval,
        #        'gamma':gamma, 'tau':tau, 'alpha':alpha, 'automatic_entropy_tuning':automatic_entropy_tuning, 'seed':seed, 'batch_size':batch_size,
        #        'num_steps':num_steps, 'warm_start_num':warm_start_num, 'num_eps':num_eps, 'hidden_size':hidden_size, 'replay_size':replay_size,
        #        'task_demos':task_demos, 'num_task_transitions':num_task_transitions, 'critic_pretraining_steps':critic_pretraining_steps,
        #        'pos_fraction':pos_fraction, 'gamma_safe':gamma_safe, 'eps_safe':eps_safe, 'tau_safe':tau_safe, 'safe_replay_size':safe_replay_size,
        #        'num_unsafe_transitions':num_unsafe_transitions, 'critic_safe_pretraining_steps':critic_safe_pretraining_steps,
        #        'MF_recovery':MF_recovery, 'Q_sampling_recovery':Q_sampling_recovery, 'ctrl_arg':ctrl_arg, 'override':override, 'recovery_policy_update_freq':recovery_policy_update_freq,
        #        'vismpc_recovery':vismpc_recovery, 'load_vismpc':load_vismpc, 'model_fname':model_fname, 'beta':beta, 'disable_offline_updates':disable_offline_updates,
        #        'disable_online_updates':disable_online_updates, 'disable_action_relabeling':disable_action_relabeling, 'add_both_transitions':add_both_transitions,
        #        'LBAC':LBAC, 'lambda_LBAC':lambda_LBAC, 'Q_risk_ablation':Q_risk_ablation, 'constraint_reward_penalty':constraint_reward_penalty,
        #        'DGD_constraints':DGD_constraints, 'use_constraint_sampling':use_constraint_sampling, 'nu':nu, 'update_nu':update_nu, 'nu_schedule':nu_schedule,
        #        'nu_start':nu_start, 'nu_end':nu_end, 'RCPO':RCPO, 'lambda_RCPO':lambda_RCPO}

        self.args=arguments(env_name, method_name, logdir, logdir_suffix, cuda, cnn, lr,
                 ...
                 LBAC, lambda_LBAC, Q_risk_ablation, constraint_reward_penalty,
                 DGD_constraints, use_constraint_sampling, nu, update_nu, nu_schedule,
                 nu_start, nu_end, RCPO, lambda_RCPO)
